...
use crab_messenger::utils::messenger::{GetInvitesRequest, GetUserChatsRequest};
use std::str::FromStr;
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut request = tonic::Request::new(GetInvitesRequest {});
    ...
    let response = client.get_invites(request).await?;
